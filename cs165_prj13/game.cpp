...
void Game :: initiate() {
   ...
   scoreTxt.setCoord(scoreMenuPoint.getX()+45, scoreMenuPoint.getY()+9);
	livesTxt.setCoord(scoreMenuPoint.getX()+45, scoreMenuPoint. getY()-21);
   title = "Asteroids";
   desc = "Break apart asteroids by shooting them and survive.";
   controls = "Arrows to move - Space to shoot - q for change to shotgun";
   scoreBoard = "Score:";
   levelBoard = "Level:";
   lifeTxt = "Life:";
}
...
void Game :: handleInput(const Interface & ui) {
   ...
   if(ui.isSpace()) {
      if(!gunType) {
         ...
         for(int i = 0; i < 15; i++) {
            Bullet tempBul;
            tempBul.fire(ship, (i*3) + ship.aimAngle()-23, tempBul.getVel().getSpeed());
            bullets.push_back(tempBul);
         }
      }
   }
