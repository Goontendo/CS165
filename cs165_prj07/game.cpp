...
void Game :: advance()
{
   if (lander.isAlive() && !lander.isLanded())
   {
      ...
      if (!ground.isAboveGround(lander.getPoint()))
      {
          ...
          lander.getVelocity();
          lander.getVelocity().setDy(0);
          lander.getVelocity().setDx(0);
          lander.setPoint(lander.getPoint());
...
void Game :: draw(const Interface & ui)
{
   ...
   if (lander.isLanded())
   {
      drawText(Point(0,0), "You have successfully landed!");
   }
   
   if (!lander.isAlive())
   {
      drawText(Point(10, 0), "You have crashed!");
   }
