...
Bird* Game :: createBird()
{
   ...
   Point point;
   int pointSelect = random(bottomRight.getY()*.5,topLeft.getY()*.5);
   int birdSelect = random(1,5);
    
   point.setX(topLeft.getX());
   point.setY(pointSelect);
    
   if (birdSelect == 1 || birdSelect == 2)
   {
      newBird = new Bird;
   }
   else if (birdSelect == 3)
   {
